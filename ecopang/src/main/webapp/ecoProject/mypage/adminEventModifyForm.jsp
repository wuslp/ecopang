...
%>
<script>
		alert("이벤트를 수정할 자격이 없습니다");
		window.location.href="/ecopang/ecoProject/news/eventContent.jsp?eve_num=<%=eve_num%>";
</script>
<%
	} else { // 세션아이디가 admin
	...
	dto = dao.getEventContent(eve_num);
	String content = dto.getEve_content().replace("\\", "&#92;");
%>
<body class="ecopang_main">
	<div id="page" class="page">
		...
		<div class="eventWriteContent">
			<form action="adminEventModifyPro.jsp" name="inputForm" method="post" enctype="multipart/form-data" onsubmit="return check()">
			<input type="hidden" value="<%=eve_num %>" name="eve_num"/>
				...
				<table class="eventWriteTable">
					...
					<tr>
						<td class="eventWriteTitle" colspan="2">
							<input type="text" name="eve_title" class="form-control" value="<%= dto.getEve_title() %>"/>
						</td>
					</tr>
					<tr>
					...
					<td class="eventWriteDate">
						<input type="date" name="eve_startdate" class="form-control" value="<%= dto.getEve_startdate() %>"/> 
					</td>
					<td class="eventWriteDate">
						<input type="date" name="eve_enddate" class="form-control" value="<%= dto.getEve_enddate() %>"/>
					</td>
					</tr>
					<tr>
						<td class="eventWriteText">
					...
					<tr>
		                        ...
		                        <td>
		                            <label for="eve__img">대표이미지</label>
		                            <input type="file" name="eve_img" id="eve_img" onchange="preview(this)" class="file"/>
		                        </td>
		                    </tr>
						<td>
						</td>
					...
					<tr>
						<td class="eventWriteBtn" colspan="2">
							...
							<input type="button" class="btn btn-primary pull-right" value="취소" onclick="javascript:history.go(-1)"></button>
						</td>
					</tr>
				</table>
			</form>
