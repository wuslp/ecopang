...
<%	
	...
	//++
	revCount = dao.getSearchRevCount(revSel,revSearch, group_num);//검색된 글의 총 개수 가져오는것
	//검색한 글이 하나라도 있으면 검색한 글 가져오기
	
	if(revCount >0){
		revList = dao.getSearchArticles(startRow, endRow, revSel, revSearch, group_num, sort);
	}
...
<script>
	...
	$(function(){
		  $("input[name=onlyPho]").click(function(){
			  alert("사진 후기만 보기");
			  $(location).attr('href',"reviewPhoto.jsp?group_num=<%= group_num %>");
		  });
		});
	...
	<header id="header" class="site-header cf">
		...
		<div class="main_content3">
			<div class="reviewSearch">
				...
				<br/>
				<span class="reviewSort"><a href="reviewAll.jsp?sort=2&pageNum=<%= pageNum %>&revSel=<%=revSel%>&revSearch=<%=revSearch%>&group_num=<%=group_num %>">인기순</a></span>
				<span class="reviewSort"><a href="reviewAll.jsp?pageNum=<%= pageNum %>&revSel=<%=revSel%>&revSearch=<%=revSearch%>&group_num=<%=group_num %>">최신순</a></span> 
				<span class="onlyPhoto">사진 후기만 보기 <input type="checkBox" name="onlyPho" value="null"></span>
			</div>
				<div class="reviewItems">
					<div text-align="left" width="100%"> 
						<input type="button" value="돌아가기" margin-left="10px" margin-right="auto"/> 
					</div>
					<%if(revList != null){
									...
									<div class="topBtns">	
										<%if(userID == null){ %>
	      								...
	      								<%}else{%>
	   									...
	   									<%} %>
										<form name="inputForm" action="reviewAll.jsp?group_num=<%=group_num%>">
											<button type="submit" onclick="report(this.form)" class="reportBtn">신고하기</button>
		...
		<% if(revCount > 0) {
			...
			if(startPage > pageBlock) { %>
				<a href="reviewAll.jsp?pageNum=<%=startPage-pageBlock %>&revSel=<%=revSel%>&revSearch=<%=revSearch%>&group_num=<%=group_num %>" class="pageNums"> &lt; &nbsp;</a>
		<%	}
			// 페이지 번호 뿌리기 
			for(int i = startPage; i <= endPage; i++){ %>
				<a href="reviewAll.jsp?pageNum=<%=i%>&revSel=<%=revSel%>&revSearch=<%=revSearch%>&group_num=<%=group_num %>" class="pageNums"> &nbsp; <%= i %> &nbsp; </a>
		<%	}
		...
		<%	}
			...
			<div id="followplus">
				<input type="button" value="+" id="button" onclick="window.location='reviewWriteFromList.jsp?group_num=<%=group_num%>'">
			</div>
		</div>
